import { Download, KeyRound, LoaderCircle, MoreHorizontal, Plus, ShieldCheck, Trash2, UserRound, Users } from 'lucide-react'
...
export function AccountsPage() {
  ...
  const [consentDate, setConsentDate] = useState(localDateKey())
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : '账号创建失败，请稍后重试')
    } finally {
      setCreating(false)
    }
  }
  ...
  const runAccountAction = async (action: () => Promise<void>, successMessage?: string) => {
    ...
    try {
      await action()
      if (successMessage) setAccountMessage(successMessage)
    } catch (reason) {
      setAccountError(reason instanceof Error ? reason.message : '账号操作失败，请稍后重试')
    } finally {
  ...
  return (
    <>
      <PageHeader title="账号管理" description="学生和家长账号由教师创建，首次登录后修改密码。" actions={<button className="button primary" type="button" onClick={() => { setOpen(true); setError('') }}><Plus size={16} /><span>新建账号</span></button>} />
      ...
      <Modal open={Boolean(selectedAccount)} title="账号操作" onClose={() => { if (!accountBusy) setSelectedAccount(null) }}>
        {selectedAccount && <div className="account-actions-panel">
          ...
          {selectedAccount.role !== 'teacher' && <div className="account-action-block"><label className="field"><span>设置新的临时密码</span><input type="password" value={newPassword} onChange={(event) => { setNewPassword(event.target.value); setAccountError(''); setAccountMessage('') }} placeholder="至少 8 位" disabled={accountBusy} /></label><button type="button" className="button" disabled={newPassword.length < 8 || accountBusy} onClick={() => void runAccountAction(async () => { await manageAccount(selectedAccount.id, 'reset_password', newPassword); setNewPassword('') }, '临时密码已更新')}><KeyRound size={15} />重置密码</button></div>}
          <div className="account-action-block"><div><strong>{selectedAccount.status === 'active' ? '停用账号' : '恢复账号'}</strong><p>停用后无法登录，历史学习数据继续保留。</p></div><button type="button" className={selectedAccount.status === 'active' ? 'button danger' : 'button'} disabled={accountBusy} onClick={() => void runAccountAction(async () => { const status = selectedAccount.status === 'active' ? 'disabled' : 'active'; await manageAccount(selectedAccount.id, 'set_status', status); setSelectedAccount({ ...selectedAccount, status }) }, selectedAccount.status === 'active' ? '账号已停用' : '账号已恢复')}>{selectedAccount.status === 'active' ? '停用' : '恢复'}</button></div>
          {selectedAccount.role === 'student' && <>
            <div className="account-action-block"><div><strong>导出已确认学情</strong><p>生成 Markdown，不包含 AI 推测和原始聊天。</p></div><button type="button" className="button" disabled={accountBusy} onClick={() => void runAccountAction(async () => { const file = await exportStudentMemory(selectedAccount.id); const url = URL.createObjectURL(new Blob([file.markdown], { type: 'text/markdown;charset=utf-8' })); const link = document.createElement('a'); link.href = url; link.download = file.fileName; link.click(); URL.revokeObjectURL(url) }, '导出已开始')}><Download size={15} />导出</button></div>
            <div className="deletion-zone">
              <div><Trash2 size={17} /><span><strong>学生数据删除</strong><small>先建立申请，再输入账号确认永久删除。</small></span></div>
              {!deletionRequestId ? <><label className="field"><span>删除原因</span><input value={deletionReason} onChange={(event) => { setDeletionReason(event.target.value); setAccountError('') }} disabled={accountBusy} /></label><button type="button" className="button danger" disabled={!deletionReason.trim() || accountBusy} onClick={() => void runAccountAction(async () => setDeletionRequestId(await requestStudentDeletion(selectedAccount.id, deletionReason.trim())))}>建立删除申请</button></> : <><p>确认文本：<code>DELETE:{selectedAccount.username}</code></p><input className="text-input" value={deletionConfirmation} onChange={(event) => { setDeletionConfirmation(event.target.value); setAccountError('') }} placeholder={`DELETE:${selectedAccount.username}`} disabled={accountBusy} /><button type="button" className="button danger" disabled={deletionConfirmation !== `DELETE:${selectedAccount.username}` || accountBusy} onClick={() => void runAccountAction(async () => { await deleteStudentData(selectedAccount.id, deletionRequestId, deletionConfirmation); setSelectedAccount(null) })}><Trash2 size={15} />永久删除学生数据</button></>}
            </div>
          </>}
          {accountBusy && <p className="form-success">正在处理，请稍候……</p>}
